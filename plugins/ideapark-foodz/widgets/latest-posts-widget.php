...
class Ideapark_Latest_Posts_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_options = [
			...
			'description' => esc_html__( 'The recent posts with thumbnails', 'ideapark-foodz' )
		];

		parent::__construct(
			'ideapark_latest_posts_widget', esc_html__( 'Custom Latest Posts', 'ideapark-foodz' ), $widget_options
		);
	}
	...
	function form( $instance ) {
		$defaults = [
			'title'         => esc_html__( 'Latest Posts', 'ideapark-foodz' ),
			'post_per_page' => 5,
			'categories'    => ''
		];
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'categories' ) ); ?>"><?php esc_html_e( 'Filter by Category:', 'ideapark-foodz' ); ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'categories' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'categories' ) ); ?>" class="widefat categories" style="width:100%;">
				<option value="all" <?php if ( $instance['categories'] == 'all' ) { ?> selected<?php } ?>><?php esc_html_e( 'All categories', 'ideapark-foodz' ); ?>
				</option>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'post_per_page' ) ); ?>"><?php esc_html_e( 'Number of posts to show:', 'ideapark-foodz' ); ?></label>
