(function ($, root, undefined) {
	...
	$(function () {
		...
		$('.ip-import-demo').change(ideapark_import_demo_check_options);

		ideapark_import_demo_check_options();

		$('#ip-import-submit').click(function (e) {
			if (importing) {
		...
		$('#ip-export-submit').click(function () {
		...
		function ideaparkSendImportRequest(data) {
			...
			$.post(ideapark_wp_vars_importer.ajaxUrl, data, function (response) {
				...
				$('.ip-import-output').html('<h3>' + ideapark_wp_vars_importer.output_error + ':</h3><div class="ip-import-error">' + jqXHR.statusText + '</div>');
				ideaparkReturnStartState();
			});
		}
