...
$post_id = sanitize_key($_GET['id']);

$meta = shiparea_get_meta($post_id);
$default = shiparea_get_default_meta();
extract($meta);

/*	Reset coords	*/
foreach($default['coords'] as $coord) {
	$symbol_lat = $coord[0]>0?'+':'';
	$symbol_lng = $coord[1]>0?'+':'';

	$array_reset_latlng[] = 'new google.maps.LatLng(cLat'.$symbol_lat.$coord[0].',cLng'.$symbol_lng.$coord[1].')';
}
...
if( !$isdefault ) {
	...
	foreach($coords as $coord) {		
		if($coord[0] != '' && $coord[1] != '')
			$array_latlng[] = 'new google.maps.LatLng('.$coord[0].','.$coord[1].')';
	}
} else
