...
if ( ! function_exists( 'ideapark_about_add_menu_items' ) ) {
	...
	function ideapark_about_add_menu_items() {
		...
		add_theme_page(
			sprintf( esc_html__( 'About %s', 'foodz' ), $theme_name ),
			sprintf( esc_html__( 'About %s', 'foodz' ), $theme_name ),
			'manage_options',
			'ideapark_about',
			'ideapark_about_page'
...
if ( ! function_exists( 'ideapark_about_enqueue_scripts' ) ) {
	...
	function ideapark_about_enqueue_scripts() {
		...
		if ( ! empty( $screen->id ) && false !== strpos( $screen->id, '_page_ideapark_about' ) ) {
			...
			wp_enqueue_script( 'ideapark-plugins-installer', IDEAPARK_THEME_URI . '/includes/theme-about/plugins-installer.js', [ 'jquery' ], null, true );
			wp_localize_script( 'ideapark-plugins-installer', 'ideapark_pi_vars', [
				'ajaxUrl'   => admin_url( 'admin-ajax.php' ),
				'errorText' => esc_html__( 'Something went wrong...', 'foodz' )
			] );
			wp_enqueue_style( 'ideapark-about', IDEAPARK_THEME_URI . '/includes/theme-about/theme-about.css', [], null );
		}
	}
}
	...
	function ideapark_about_ajax() {
		...
		if ( ! empty( $_POST['is_additional'] ) ) {
			...
			if ( ! empty( $other_plugin_action ) ) {
				...
				ob_start();
				ideapark_additional_plugin_list( $other_plugin_list );
				$list = ob_get_clean();
				echo json_encode(
					[
						'success' => true,
		...
		foreach ( $plugins AS $plugin ) {
			...
			if ( in_array( $plugin['state'], [ 'install', 'activate', 'update' ] ) ) {
				if ( $is_required ) {
					if ( ! $next_action ) {
						$next_action = [
							'name'   => sprintf( $plugin['state'] == 'install' ? __( 'Install %s', 'foodz' ) : ( $plugin['state'] == 'update' ? __( 'Update %s', 'foodz' ) : __( 'Activate %s', 'foodz' ) ), $plugin['name'] ),
							'slug'   => $plugin['slug'],
							'state'  => $plugin['state'],
							'action' => ideapark_plugins_installer_get_action( $plugin )
						];
					}
					if ( ( $plugin['slug'] == 'ideapark-foodz' ) ) {
					...
					if ( ! $other_plugin_action && in_array( $plugin['slug'], $filter ) ) {
						$other_plugin_action = [
							'name'   => ( $plugin['state'] == 'install' ? __( 'Install', 'foodz' ) : ( $plugin['state'] == 'update' ? __( 'Update', 'foodz' ) : __( 'Activate', 'foodz' ) ) ) . ' ' . $plugin['name'],
							'slug'   => $plugin['slug'],
							'state'  => $plugin['state'],
							'action' => ideapark_plugins_installer_get_action( $plugin )
						];
					}
					$other_plugin_list[ $plugin['slug'] ] = $plugin['name'] . ideapark_wrap( $plugin['state'] == 'install' ? __( 'Install and activate', 'foodz' ) : ( $plugin['state'] == 'update' ? __( 'Update', 'foodz' ) : __( 'Activate', 'foodz' ) ), '<span class="action_name">', '</span>' );
				}
			}
		}
...
if ( ! function_exists( 'ideapark_about_page' ) ) {
	function ideapark_about_page() {
		...
		<div class="ideapark_about">
			<div class="ideapark_about_logo">
				<img src="<?php echo esc_url( get_template_directory_uri() ) ?>/assets/img/logo.svg" class="svg"
					 alt="Theme logo"/>
			</div>
				...
				<div class="ideapark_about_required_plugins">
					<div class="ideapark_about_description">
						<p>
							...
							echo wp_kses_post(
								sprintf(
									__( 'In order to continue, please install and activate or update required plugins:<br><b>%1$s</b>', 'foodz' ),
					...
					<div class="ideapark_about_buttons">
						<a class="ideapark_plugins_installer_link button button-primary install-now"
						   href="#"><?php esc_html_e( 'Continue', 'foodz' ) ?></a>
						<div class="ideapark_plugins_installer_error"></div>
						<div class="ideapark_plugins_installer_success hidden">
							<p>
								<span
									class="dashicons dashicons-yes"></span> <?php echo wp_kses_post( __( 'All required plugins have been successfully installed.', 'foodz' ) ); ?>
							</p>
						</div>
					</div>
					...
					<?php if ( $main_plugin_name ) { ?>
						<div class="ideapark_about_notes">
							<p>
								...
								echo esc_html(
									sprintf(
										__( '%1$s plugin will allow you to import demo content, and improve the theme\'s functionality', 'foodz' ),
										$main_plugin_name
									)
								);
								?>
			...
			<div class="ideapark_about_next_step <?php if ( $plugin_names ) { ?>hidden<?php } ?>">
				<?php if ( current_user_can( 'install_plugins' ) ) { ?>
					<div class="step">
						<b><?php esc_html_e( 'Additional plugins', 'foodz' ); ?></b>
						<p><?php esc_html_e( 'You can install additional plugins to extend the functionality of the theme', 'foodz' ); ?></p>
						<?php if ( ! empty( $other_plugin_list ) && ideapark_plugins_installer_tgmpa_menu() ) { ?>
							<?php ideapark_additional_plugin_list( $other_plugin_list ); ?>
							<a href="<?php echo admin_url( 'themes.php?page=tgmpa-install-plugins' ); ?>"
							   class="ideapark_plugins_installer_link additional button button-primary"><?php esc_html_e( 'Continue', 'foodz' ); ?></a>
						<?php } ?>
						<p class="additional-plugins-installed <?php if ( ! empty( $other_plugin_list ) && ideapark_plugins_installer_tgmpa_menu() ) { ?> hidden<?php } ?>">
							<b><?php echo wp_kses_post( __( 'All additional plugins are installed.', 'foodz' ) ); ?></b>
						</p>
					</div>
				<?php } ?>
				<div class="step">
					<b><?php esc_html_e( 'Import Demo', 'foodz' ); ?></b>
					<p><?php esc_html_e( 'Use our Demo Content Importer to make your website similar to our demos', 'foodz' ); ?>
						<br><br><b><?php esc_html_e( 'Please install necessary additional plugins before importing', 'foodz' ); ?></b>
					</p>
					<a href="<?php echo admin_url( 'themes.php?page=ideapark_themes_importer_page' ); ?>"
					   class="button button-primary"><?php esc_html_e( 'Import demo', 'foodz' ); ?></a>
				</div>
				<div class="step">
					<b><?php esc_html_e( 'Theme customization', 'foodz' ); ?></b>
					<p><?php echo wp_kses_post( sprintf( __( 'Explore the documentation and then start customizing the theme', 'foodz' ) ) ); ?></p>
					<?php /*?><p><?php echo wp_kses_post( sprintf( __( 'Explore the <a href="%s" target="_blank">documentation</a> and then start customizing the theme', 'foodz' ), IDEAPARK_THEME_MANUAL ) ); ?></p><?php */ ?>
					<a href="<?php echo admin_url( 'customize.php' ); ?>"
					   class="button button-primary"><?php esc_html_e( 'Customize', 'foodz' ); ?></a>
					<p>
						<a style="color: #fff"
						   href="<?php echo admin_url( 'themes.php?page=ideapark_about&clear_cache&noheader' ); ?>"><?php esc_html_e( 'Clear theme cache', 'foodz' ); ?></a>
					</p>
				</div>
			</div>
		</div>
...
if ( ! function_exists( 'ideapark_additional_plugin_list' ) ) {
	function ideapark_additional_plugin_list( $other_plugin_list ) {
		$filter = false;
				...
				<?php foreach ( $other_plugin_list as $plugin_code => $plugin_name ) { ?>
					<li class="plugins_list__item">
						<label><input type="checkbox" class="ideapark_additional_plugin" name="plugin[]"
						              ...
						              <?php if ( $filter === false || in_array( $plugin_code, $filter ) ) { ?>checked<?php } ?>> <?php echo wp_kses_post( $plugin_name ); ?>
						</label>
					</li>
				<?php } ?>
			</ul>
