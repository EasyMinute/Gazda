(function ($) {
	...
	window.IP_Wishlist = {
		...
		init: function () {
			...
			$(document).on('click', '.c-wishlist__btn', function (e) {
				e.preventDefault();
				...
				$buttons.ideapark_button('loading');
			...
			if ($wishlistTable.length) {
					...
					$.ajax({
						...
						success : function (json) {
							...
							if (json.count > 0) {
								...
								$('#ip-wishlist-empty').removeClass('h-hidden');
							}
				...
				$wishlistTable.on('click', '.c-wishlist__remove', function (e) {
					e.preventDefault();
