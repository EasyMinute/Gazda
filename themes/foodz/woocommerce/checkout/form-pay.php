					...
					<?php if ( count( $order->get_items() ) > 0 ) : ?>
						<?php foreach ( $order->get_items() as $item_id => $item ) : ?>
							...
							<tr class="c-cart__shop-tr <?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $order ) ); ?>">
								...
								<td class="c-cart__shop-td c-cart__shop-td--product-subtotal"><?php echo $order->get_formatted_line_subtotal( $item ); ?></td><?php // @codingStandardsIgnoreLine ?>
							</tr>
						<?php endforeach; ?>
					<?php endif; ?>
					</tbody>
					...
					<?php if ( $totals ) : ?>
						<?php foreach ( $totals as $total ) : ?>
							<tr class="c-cart__shop-tr">
								<th class="c-cart__shop-td c-cart__shop-td--left" scope="row"
									colspan="2"><?php echo $total['label']; ?></th><?php // @codingStandardsIgnoreLine ?>
								<td class="c-cart__shop-td c-cart__shop-td--product-subtotal"><?php echo $total['value']; ?></td><?php // @codingStandardsIgnoreLine ?>
							</tr>
						<?php endforeach; ?>
					<?php endif; ?>
					</tfoot>
			...
			<div
				...
				<div id="payment">
					...
					<div class="c-cart__place-order form-row">
						...
						<?php echo apply_filters( 'woocommerce_pay_order_button_html', '<button type="submit" class="c-form__button c-cart__place-order-btn button alt" id="place_order" value="' . esc_attr( $order_button_text ) . '" data-value="' . esc_attr( $order_button_text ) . '">' . esc_html( $order_button_text ) . '</button>' ); // @codingStandardsIgnoreLine ?>
